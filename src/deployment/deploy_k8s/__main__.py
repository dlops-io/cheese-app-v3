...
network, subnet, router, nat = create_network(region)

# Create & Setup Cluster
cluster, namespace, k8s_provider, ksa_name = create_cluster(
    project, region, network, subnet
)
...
nginx_ingress_ip, ingress, host = setup_loadbalancer(
    namespace, k8s_provider, api_service, frontend_service
)
